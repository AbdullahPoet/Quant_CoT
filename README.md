...
Before treating the benchmark as final:

1. Fix Math500 final-answer extraction.
2. Rescore saved generations without rerunning inference.
3. Recompute aggregate and per-dataset accuracy.
4. Re-run McNemar tests using the corrected correctness labels.
5. Analyze reasoning-token distributions rather than only means.
6. Compare correct vs incorrect examples by reasoning length.
7. Quantify token-budget exhaustion separately from ordinary answer errors.
8. Inspect BF16 ↔ quantized answer-flip cases.
9. Add confidence intervals for the main metrics.
10. Perform qualitative analysis of representative overthinking and truncation failures.

---

## Preliminary Takeaway

> **Quantizing DeepSeek-R1-Distill-Qwen-7B substantially reduces model size and can improve inference throughput without producing a proportional loss in measured accuracy. In this run, Q5_K_M retained BF16-level aggregate measured accuracy while reducing artifact size by roughly 64%. More aggressive Q4/Q3 quantization increased reasoning-token usage and truncation without improving accuracy, providing evidence that longer reasoning traces are not inherently better.**

These conclusions are **preliminary** until the Math500 parsing issue is corrected and the saved outputs are rescored.
